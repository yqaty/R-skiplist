...
#define do_log(log_type, fmt, arg...)                                     \
  fprintf(stderr, #log_type " [" __FILE__ ":%d] %s: " fmt "\n", __LINE__, \
          __FUNCTION__, ##arg)
...
constexpr bool is_power_of2(T val) noexcept {
  return val >= 0 && (val & (val - 1)) == 0;
}

template <typename T>
  requires std::is_integral_v<T>
constexpr bool is_times_ofN(
    T val, std::type_identity_t<T> N = cache_line_size) noexcept {
  return val >= 0 && N > 0 && (val & (N - 1)) == 0;
}

template <typename T>
  requires std::is_integral_v<T>
constexpr T upper_align(T val,
                        std::type_identity_t<T> align = cache_line_size) {
  assert_check(val >= 0);
...
#define tmap(expr, src_tuple) \
  std::apply([&](auto... x) { return std::make_tuple((expr)...); }, src_tuple)
...
#else
  if (posix_memalign(&res, 1 << (page_flags >> MAP_HUGE_SHIFT), size))
      [[unlikely]] {
...
class tempmp_impl : noncopyable {
  ...
  enum { flag_freed, flag_inuse, flag_gc, flag_invaild };
  struct ctrl_block {
    uint32_t flag : 2;
    uint32_t len : 30;
  };
  ...
  auto alloc_many_impl(uint64_t base, T val, Ts... size) {
    return std::tuple_cat(
        std::make_tuple((void *)base),
        alloc_many_impl(base + upper_align(val, align), size...));
  }

 public:
  tempmp_impl(size_t _size)
      : head(0), tail(0), blk_num(_size / align), blk_cap(_size / align - 1) {
    assert_require(is_power_of2(_size) && _size >= (1 << 21) &&
                   (_size / align) <= (size_t)(1u << 31));
    assert_require(data = (uint8_t *)alloc_hugepage(_size));
    assert_require(ctl_blks =
                       (ctrl_block *)calloc(blk_num, sizeof(ctrl_block)));
  }
  ~tempmp_impl() {
    if (data) free_hugepage(data, get_data_len());
    if (ctl_blks) ::free(ctl_blks);
  }
  ...
  void *alloc(uint32_t size) {
    ...
    if (real_blk > free_blk) return nullptr;
...
inline void tempmp_free(void **ptr) {
  (*(tempmp **)((uint8_t *)*ptr - cache_line_size))->free(*ptr);
}
...
class cycle_queue : noncopyable {
 ...
 public:
  ...
  cycle_queue(cycle_queue &&oth)
      : data(std::move(oth.data)),
        head(std::move(oth.head)),
        tail(std::move(oth.tail)),
        cap(std::move(oth.cap)) {}
  ~cycle_queue() {
    free_hugepage(data, upper_align((cap + 1) * sizeof(T), 1 << 21));
  }
  void enqueue(T &&t) {
...
class unique_int_hashmap {
 ...
 public:
  ...
  void lockfree_insert(uint32_t key, ValueType val) {
    ...
    for (uint32_t i = 0; i < search_cap;
         ++i, (hkey = (hkey + 1) & (size - 1))) {
      if (data[hkey].key == 0) {
        __cas_placeholder = 0;
        if (!__atomic_compare_exchange_n(&data[hkey].key, &__cas_placeholder,
  ...
  ValueType search(uint32_t key) {
    uint32_t hkey = key & (size - 1);
    for (uint32_t i = 0; i < search_cap; ++i, (hkey = (hkey + 1) & (size - 1)))
      if (data[hkey].key == key) return data[hkey].val;
    return nullptr;
  }
};
...
struct task_result {
  constexpr bool has_value() const noexcept {
    return std::get_if<std::monostate>(&__result) == nullptr;
  }
  constexpr bool has_error() const noexcept {
    return std::get_if<std::exception_ptr>(&__result) != nullptr;
  }
  template <typename R>
  constexpr void return_value(R &&value) noexcept {
    __result.template emplace<T>(std::forward<R>(value));
  }
  void unhandled_exception() noexcept { __result = std::current_exception(); }
  constexpr T result() & {
    if (auto res = std::get_if<T>(&__result)) [[likely]]
      return *res;
    if (auto exception = std::get_if<std::exception_ptr>(&__result))
        [[unlikely]]
...
struct task_result<void> {
  ...
  constexpr bool has_error() const noexcept {
    return (__result.has_value() && *__result != nullptr);
  }
...
struct [[nodiscard]] task : private noncopyable {
  ...
  task &operator=(task &&t) {
    __handle = std::exchange(t.__handle, {});
    return *this;
  }
  ~task() { destroy(); }
  ...
  decltype(auto) get_result() && {
    return std::move(__handle.promise()).result();
  }

  struct final_awaiter {
    constexpr bool await_ready() noexcept { return false; }
    template <typename PromiseType>
    constexpr decltype(auto) await_suspend(
        std::coroutine_handle<PromiseType> h) noexcept {
      return h.promise().__previous
                 ? (std::coroutine_handle<>)h.promise().__previous
                 : std::noop_coroutine();
    }
  ...
  struct awaiter_base {
    ...
    auto await_suspend(std::coroutine_handle<Promise> h) const noexcept {
      return (__coro.promise().__previous = h), __coro;
    }
  ...
  auto operator co_await() & noexcept {
    struct awaiter : awaiter_base {
      decltype(auto) await_resume() const {
        return awaiter_base::__coro.promise().result();
      }
    };
  ...
  auto operator co_await() && noexcept {
    struct awaiter : awaiter_base {
      decltype(auto) await_resume() const {
        return std::move(awaiter_base::__coro.promise()).result();
      }
    };
    // return awaiter{std::exchange(__handle, {})};
    return awaiter{__handle};
  ...
  void start(F &&callback) {
    struct start_awaiter : awaiter_base {
      decltype(auto) await_resume() const {
        return std::move(awaiter_base::__coro.promise());
      }
    };
...
template <typename A>
using get_result_t =
    task_result<decltype(std::declval<get_awaiter_t<A>>().await_resume())>;
...
struct try_resumer {
  ...
  void operator()() {
    if (++__ready == __target) resume_handler();
  }
};
  ...
  [](T &t, task_result<R> &r, F &f) -> undelay_task {
    try {
      if constexpr (std::is_void_v<R>) {
        ...
        r.__result.template emplace<R>(std::move(co_await t));
    } catch (...) {
...
struct [[nodiscard]] gather_awaiter {
  using result_type = std::conditional_t<is_val, get_value_t<AwaitableTypes>,
                                         get_result_t<AwaitableTypes>>;
  static_assert(!std::is_same_v<result_type, bool>);
  gather_awaiter(std::vector<AwaitableTypes> &&input_tasks)
      : __tasks(std::move(input_tasks)) {
    __task_results.resize(__tasks.size());
  }
  ...
  void await_suspend(std::coroutine_handle<> h) {
    ...
    for (size_t i = 0; i < __tasks.size(); ++i)
      if constexpr (is_val)
        gather_start_value(__tasks[i], __task_results[i], __resumer);
      else
        gather_start(__tasks[i], __task_results[i], __resumer);
    __resumer();
  }
...
[[nodiscard]] task<std::vector<get_result_t<AwaitableTypes>>> gather(
    std::vector<AwaitableTypes> &&input_awaitables) {
  co_return co_await gather_awaiter<false, AwaitableTypes>(
      std::forward<std::vector<AwaitableTypes>>(input_awaitables));
}

template <typename AwaitableTypes>
[[nodiscard]] task<std::vector<get_value_t<AwaitableTypes>>> gather_value(
    std::vector<AwaitableTypes> &&input_awaitables) {
  co_return co_await gather_awaiter<true, AwaitableTypes>(
      std::forward<std::vector<AwaitableTypes>>(input_awaitables));
}
...
template <typename... AwaitableTypes>
[[nodiscard]] task<std::tuple<get_result_t<AwaitableTypes>...>> gather(
    AwaitableTypes &&...input_awaitables) {
  co_return co_await gather_tuple_awaiter(
      std::forward<AwaitableTypes>(input_awaitables)...);
}

template <typename... AwaitableTypes>
[[nodiscard]] task<std::tuple<get_value_t<AwaitableTypes>...>> gather_value(
    AwaitableTypes &&...input_awaitables) {
  co_return tmap(x.result(),
                 co_await gather_tuple_awaiter(
                     std::forward<AwaitableTypes>(input_awaitables)...));
}